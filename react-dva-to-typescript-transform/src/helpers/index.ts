...
export function isReactComponent(
  ...
  if (!/React\.Component|Component|PureComponent/.test(typeSymbol)) {
    return false;
  }
 ...
 * @param statement
 */
export function isReactPropTypeAssignmentStatement(
...
export function getTypeNode(text: string, typeString: string, type: any, typeChecker?: ts.TypeChecker) {
  let typeNode;
  if (text.indexOf("[") >= 0 || typeString.indexOf("[") >= 0) {
    ...
    typeNode = ts.createKeywordTypeNode(ts.SyntaxKind.NullKeyword);
  } else if (
    typeString === "Timer" ||
  ...
  } else if (typeString === '\'\'' || (/[0-9A-Za-z]/.test(typeString))) {
    typeNode = ts.createKeywordTypeNode(ts.SyntaxKind.StringKeyword);
  } else if (type && typeChecker) {
 ...
 * @param sourceFile
 */
export function isPropTypesMember(
  classMember: ts.ClassElement,
  sourceFile: ts.SourceFile
) {
  try {
    const name =
      classMember.name !== undefined && ts.isIdentifier(classMember.name)
...
export function getComponentName(
  propTypeAssignment: ts.Statement,
  sourceFile: ts.SourceFile
) {
