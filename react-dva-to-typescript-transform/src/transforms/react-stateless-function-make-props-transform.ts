 ...
 * }
 */
export function reactStatelessFunctionMakePropsTransformFactoryFactory(
  typeChecker: ts.TypeChecker
): Factory {
  return function reactStatelessFunctionMakePropsTransformFactory(
    context: ts.TransformationContext
  ) {
    return function reactStatelessFunctionMakePropsTransform(
...
function visitSourceFile(
  ...
  for (const propTypeAssignment of propTypeAssignments) {
    const componentName = helpers.getComponentName(
      propTypeAssignment,
      newSourceFile
    );
    ...
    const funcComponent = (_.find(newSourceFile.statements, (s) => {
      return (
        (ts.isFunctionDeclaration(s) &&
          s.name !== undefined &&
          s.name.getText() === componentName) ||
        (ts.isVariableStatement(s) &&
          s.declarationList.declarations[0].name.getText() === componentName)
      );
    }) as {}) as ts.FunctionDeclaration | ts.VariableStatement; // Type weirdness

    if (funcComponent) {
      newSourceFile = visitReactStatelessComponent(
        funcComponent,
        propTypeAssignment,
        newSourceFile
      );
...
function visitReactStatelessComponent(
  ...
  const arrowFuncComponent = helpers.convertReactStatelessFunctionToArrowFunction(
    component
  );
  const componentName = arrowFuncComponent.declarationList.declarations[0].name.getText();
  const componentInitializer =
    arrowFuncComponent.declarationList.declarations[0].initializer;
  ...
  const propTypeName = `${componentName}PropsType`;
  ...
  const componentType = ts.createTypeReferenceNode(
  ...
  let statements = shouldMakePropTypeDeclaration
    ? helpers.insertBefore(sourceFile.statements, component, [
        propTypeDeclaration
      ])
...
function getPropTypesFromTypeAssignment(
  propTypesExpressionStatement: ts.ExpressionStatement
) {
  if (
    ...
    return helpers.buildInterfaceFromPropTypeObjectLiteral(
      propTypesExpressionStatement.expression.right
    );
  }
