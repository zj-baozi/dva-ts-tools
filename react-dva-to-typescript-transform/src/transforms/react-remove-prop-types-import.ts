 ...
 */
export function reactRemovePropTypesImportTransformFactoryFactory(
  typeChecker: ts.TypeChecker
): Factory {
  return function reactRemovePropTypesImportTransformFactory(
    context: ts.TransformationContext
  ) {
    return function reactRemovePropTypesImportTransform(
...
function updateReactImportIfNeeded(statement: ts.Statement) {
  ...
  const newNamedBindingElements = namedBindings.elements.filter(
    (elm) => elm.name.text !== "PropTypes"
  );
