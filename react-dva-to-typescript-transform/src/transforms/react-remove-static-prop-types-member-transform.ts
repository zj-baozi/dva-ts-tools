 ...
 * class SomeComponent extends React.Component<{foo: number;}, {bar: string;}> {}
 */
export function reactRemoveStaticPropTypesMemberTransformFactoryFactory(
  typeChecker: ts.TypeChecker
): Factory {
  return function reactRemoveStaticPropTypesMemberTransformFactory(
    context: ts.TransformationContext
  ) {
    return function reactRemoveStaticPropTypesMemberTransform(
      ...
      function visitor(node: ts.Node) {
        if (
          ...
          helpers.isReactComponent(node, typeChecker)
        ) {
