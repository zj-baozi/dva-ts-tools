 ...
 * ```
 */
export function reactMovePropTypesToClassTransformFactoryFactory(
  typeChecker: ts.TypeChecker
): Factory {
  return function reactMovePropTypesToClassTransformFactory(
    context: ts.TransformationContext
  ) {
    return function reactMovePropTypesToClassTransform(
...
function visitSourceFile(
  ...
  for (const propTypeAssignment of propTypeAssignments) {
    ...
    if (
      classStatement &&
      ts.isBinaryExpression(propTypeAssignment.expression)
    ) {
      ...
      statements = ts.createNodeArray(
        helpers.replaceItem(statements, classStatement, newClassStatement)
      );
    }
  }
