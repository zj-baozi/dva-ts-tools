...
function visitSourceFile(
  ...
  helpers.visitor(sourceFile.statements, (statement) => {
    if (ts.isObjectLiteralExpression(statement)) {
      ...
      const variableDeclationList = variableDeclation && variableDeclation.parent;
      const variableStatements = variableDeclationList && variableDeclationList.parent;
      ...
      const namespaceModelType =  nameSpace ? ts.createPropertySignature(
        undefined,
      ...
      const stateModelType = stateType ? ts.createPropertySignature(
        undefined,
        ts.createIdentifier("state"),
        undefined,
        ts.createTypeReferenceNode(ts.createIdentifier(STATE_TYPE_NAME), stateType),
        undefined
      ) : undefined;

      const effectModelType = effectType ? ts.createPropertySignature(
        undefined,
      ...
      const reducersModelType = reducersType ? ts.createPropertySignature(
        undefined,
      ...
      const defaultExportAssignment = variableStatement || (_.find(
        sourceFile.statements,
        (item) =>
          ts.isExportAssignment(item)) as {});
    ...
    .map((item) => {
      return ts.createPropertySignature(
        ...
        ts.createTypeReferenceNode(ts.createIdentifier("Reducer"), [
          ts.createTypeReferenceNode(ts.createIdentifier(STATE_TYPE_NAME), undefined)
        ]),
        undefined
      );
    });
