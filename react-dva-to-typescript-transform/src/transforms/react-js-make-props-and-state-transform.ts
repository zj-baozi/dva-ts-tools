...
export function reactJSMakePropsAndStateInterfaceTransformFactoryFactory(
  ...
  return function reactJSMakePropsAndStateInterfaceTransformFactory(
    ...
    return function reactJSMakePropsAndStateInterfaceTransform(
...
function visitSourceFile(
  ...
  for (const statement of sourceFile.statements) {
    if (
      ...
      newSourceFile = visitReactClassDeclaration(
        statement,
        newSourceFile,
        typeChecker
      );
...
function visitReactClassDeclaration(
  ...
  const propType = getPropsTypeOfReactComponentClass(
    classDeclaration,
    sourceFile
  );
  const stateType = getStateTypeOfReactComponentClass(
    classDeclaration,
    typeChecker
  );
  ...
  const propTypeName = `${className}PropsType`;
  const stateTypeName = `${className}StateType`;
  const propTypeDeclaration = ts.createTypeAliasDeclaration(
    [],
    [],
    propTypeName,
    [],
  ...
  statements = helpers.replaceItem(
    statements,
    classDeclaration,
    newClassDeclaration
  );
...
function getPropsTypeOfReactComponentClass(
  ...
  >(classDeclaration.members, (node) => {
    return ts.isBinaryExpression(node) || ts.isPropertyAccessExpression(node) || ts.isVariableDeclaration(node);
  });

  const type = ts.createKeywordTypeNode(ts.SyntaxKind.AnyKeyword);
  let totalText = '';
  expressions.forEach((expression: ts.Expression) => {
    if (expression.getText().indexOf("this.props") > -1) {
      expression.getChildren().forEach((item: any) => {
        const itemText = item.getText();
        if ((itemText.indexOf("this.props") < 0  && itemText.indexOf("=") < 0) &&
        (itemText.indexOf("this") < 0 && itemText.indexOf("prop") < 0 && itemText.indexOf(".") < 0)) {
          item.getChildren().forEach((expressionItem: ts.Expression) => {
            expressionItem.getChildren().forEach((bindingElement: any) => {
              let text = bindingElement.getText();
              if (text !== ',') {
                if (text.indexOf('{') > -1) {
                  text = text.split(':')[0];
                }
  ...
  if (
    ...
    // @ts-ignore
    arr = arr.concat(
      helpers.buildInterfaceFromPropTypeObjectLiteralMember(
        staticPropTypesMember.initializer
      )
    );
    // @ts-ignore
    return ts.createTypeLiteralNode(arr);
  }

  const staticPropTypesGetterMember = _.find(
    classDeclaration.members,
    (member) => {
      return (
