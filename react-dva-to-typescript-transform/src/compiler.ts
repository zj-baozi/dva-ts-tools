...
export function compile(
  ...
  const sourceFiles = program
    .getSourceFiles()
    .filter((sf) => sf.fileName === filePath);
  const typeChecker = program.getTypeChecker();

  const result = ts.transform(
    sourceFiles,
    factoryFactories.map(
      (factoryFactory) => factoryFactory(typeChecker),
      compilerOptions
    )
  );
  ...
  try {
    ...
    if (compilationOptions.ignorePrettierErrors) {
      console.warn(
        `Prettier failed for ${filePath} (ignorePrettierErrors is on):`
      );
...
function getCodeWidth(source: string, defaultWidth: number): number {
  return source
    .split(os.EOL)
    .reduce((result, line) => Math.max(result, line.length), defaultWidth);
}
